...
void cpl_drawEdit() {
	...
	for (int i = 0; i < w_chunksSize; i++) {
		...
		for (int j = 0; j < 64; j++) {
			...
			if (i == w_currX && j == w_currY) {
				cpl_drawTile(1, x, y);
			}

			if (i == w_start.x && j == w_start.y) {
				cpl_drawTile(99, x, y);
			}
		}
	}
...
void cpl_updateView() {
	if (w_currY < 0) w_currY = 0;
	if (w_currY > 63) w_currY = 63;
	if (w_currX < 0) w_currX = 0;
	if (w_currX > w_chunksSize - 1) w_currX = w_chunksSize - 1;
